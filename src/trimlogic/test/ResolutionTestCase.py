...
from trimlogic.predicate import RuleBasedPredicate, Rule, Fact
...
from trimlogic.algorithm import fol_bc_ask, unify
from trimlogic.stdlib import eql
...
class ResolutionTestCase(unittest.TestCase):
  ...
  def testOccursCheck(self):
    v, a = self.v, self.a
    f = RuleBasedPredicate('f')
    f.rules.append( Fact(f, (v.X,)) )
    m = {}
    for x in fol_bc_ask([eql(v.X, f(v.X)), eql(v.Y, f(v.Y)), eql(v.X, v.Y)], m):
      print str(x)
